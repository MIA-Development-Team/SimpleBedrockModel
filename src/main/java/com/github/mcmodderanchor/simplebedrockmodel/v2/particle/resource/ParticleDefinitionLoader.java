package com.github.mcmodderanchor.simplebedrockmodel.v2.particle.resource;

import com.github.mcmodderanchor.simplebedrockmodel.SimpleBedrockModel;
import com.github.mcmodderanchor.simplebedrockmodel.v2.particle.data.ParticleEffectDefinition;
import com.github.mcmodderanchor.simplebedrockmodel.v2.particle.data.ParticleEffectDeserializer;
import com.github.mcmodderanchor.simplebedrockmodel.v2.particle.runtime.ParticleMolangEnvironment;
 ...
 */
public class ParticleDefinitionLoader extends SimplePreparableReloadListener<Map<Identifier, JsonElement>> {
    ...
    private final Map<Identifier, ParticleEffectDefinition> cache = Maps.newHashMap();
    private final Map<Identifier, ParticleEffectDefinition> identifierIndex = Maps.newHashMap();
    ...
    protected Map<Identifier, JsonElement> prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        Map<Identifier, JsonElement> result = Maps.newHashMap();
        resourceManager.listResources(DIRECTORY, loc -> loc.getPath().endsWith(".json"))
                .forEach((location, resource) -> {
                    try (InputStream stream = resource.open()) {
                        ...
                        Identifier id = Identifier.fromNamespaceAndPath(location.getNamespace(), name);
                        result.put(id, json);
                    } catch (IOException e) {
    ...
    @Nullable
    public ParticleEffectDefinition getDefinition(Identifier id) {
        ParticleEffectDefinition def = cache.get(id);
        if (def != null) return def;
        return identifierIndex.get(id);
    }

    public Map<Identifier, ParticleEffectDefinition> getAllDefinitions() {
